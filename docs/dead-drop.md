...
Dead Drop mode encrypts your release packet and uploads it to an S3-compatible bucket while you are alive. Your contacts download the packet after a release event — even if your local machine is offline.
...
**Limitation:** Dead Drop without Relay Escrow only protects against upload-time host loss. For full offline release capability, Relay Escrow (Phase 4+) holds the key in escrow.
